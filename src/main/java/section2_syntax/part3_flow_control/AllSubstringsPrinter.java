...
public class AllSubstringsPrinter {
    ...
    public void printAllSubstrings(String stringToSubstring, boolean leftTruncated, boolean leftAligned) {
        ...
        for (int i = 0; i < stringToSubstring.length(); i++) {
            ...
            if (leftTruncated)
                substring.append(stringToSubstring, i, stringToSubstring.length());
            else
                substring.append(stringToSubstring.substring(0 , stringToSubstring.length() - i));
            if (!leftAligned)
                substring.insert(0, createSpacer(i));
